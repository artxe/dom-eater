...
import parse_element from "../index.js"
...
export default function(text, errors, start, bracket) {
	if (text[start] == "{") {
		const node = parse_script_block(text, errors, start, bracket)
		return {
			end: node.end,
			name: "",
			start,
	...
	if (name_index > 0) {
		...
		if (value_index > 0) {
			...
			if (text[index] == "'" || text[index] == "\"") {
				const value = parse_attribute_quotes(text, errors, index, bracket)
				node.value = value
				node.end = value.end
			} else if (text[index] == "{") {
				const value = parse_script_block(text, errors, index, bracket)
				node.value = value
				node.end = value.end
			} else if (text[index] == "<") {
				const element = parse_element(text, errors, index, bracket)
				node.value = {
					elements: [ element ],
					end: element.end,
					start: index,
